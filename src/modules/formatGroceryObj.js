...
const formatGroceryObj = (resultObj = {}) => {
  ...
  const itemImage = valueFromPossibleKeys(resultObj, "image", "image_url", "image_small_url") || getImageFromList(resultObj) || missingImage
  ...
  const itemPurchaseSize = `${valueFromPossibleKeys(resultObj, "purchase_size", "quantity", "net_weight_value", "net_weight_unit", "volume_unit", "serving_size") || itemQuantity || grocerySchema.purchase_size}`
  ...
  const itemPurchasePrice = valueFromPossibleKeys(resultObj, "purchase_price", "unit_price", "cost", "price") || grocerySchema.purchase_price
  const itemUnitSize = valueFromPossibleKeys(resultObj, "unit_size", "quantity", "net_weight_value", "net_weight_unit", "volume_unit", "serving_size", "purchase_size") || grocerySchema.unit_size
  ...
  const ItemServingQuantity = valueFromPossibleKeys(resultObj, "serving_quantity", "number_of_servings") || grocerySchema.serving_quantity

  const formattedObj = {
    name: toTitleCase(shortenedName),
    image: itemImage,
    priority: itemPriority,
    purchase_size: itemPurchaseSize.trim(),
    ...
    serving_size: itemServingSize.trim(),
    serving_quantity: ItemServingQuantity,
    unit_size: itemUnitSize.trim(),
  };
